...
//piece
    ...
    Move(square) {
        ...
        // mean is this is attack mean holder stand, and this holder not from target or other class which is enemy of random current turn? 
        console.log("what son of noob" );
              

        if (isAttack && !(character instanceof this.getOtherClass())){
          return alert("Oh: You can not attack your ally its not bbug bomb feature ClientMessage Class needed");
        
          //return new ClientError(false, "Its not your turn this ClientMessage Class", 'Game.startMove', null, this);
        }
        // black != white true  , black != black false 
        if ( this.turn === this.getOtherTurn() ) {
          return alert("Oh this triky socket errro you can not move on other player turn");
        }
        
        
               // temp here added shouldadded last func must added before switch turn
             if ( isAttack ) {
                 alert("handle attack");
